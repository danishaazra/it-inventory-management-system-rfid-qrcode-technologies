...
async function fetchAssetDetails(assetId) {
  try {
    ...
    const url = `/api/assets/get-by-assetid?assetId=${encodeURIComponent(assetId)}&staffId=${encodeURIComponent(staffId)}`;
    ...
    if (!resp.ok || !data.ok) {
      ...
      // show a clear popup message with assigned staff info.
      if (data && (data.error === 'ASSET_NOT_ASSIGNED_TO_STAFF' || resp.status === 403)) {
        ...
        showAssignmentError(msg, assignedStaffName, assignedStaffEmail);
        if (scanResult) scanResult.classList.remove('show');
        return;
      } else {
