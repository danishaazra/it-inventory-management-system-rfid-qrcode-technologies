...
// - setupInspectionActions (Edit/Assign/Delete menu)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Staff version - removed admin-only functions (setupInspectionActions, setupEditMaintenanceModal, setupAssignStaffModal)
...
async function loadMaintenanceItemFromURL() {
  ...
  // Support both 'id' (admin) and 'maintenanceId' (staff) parameters
  const maintenanceId = urlParams.get('id') || urlParams.get('maintenanceId');
  const branch = urlParams.get('branch');
  const location = urlParams.get('location');
  const itemName = urlParams.get('itemName');
  const frequency = urlParams.get('frequency');
  
  // If we have maintenanceId, we can load the rest from the API
  if (maintenanceId) {
    // Load full maintenance item details first
    try {
      ...
      if (response.ok) {
        ...
        if (data.ok && data.maintenance) {
          ...
          // Set current maintenance item with all data from API
          currentMaintenanceItem = {
            _id: maintenance._id || maintenanceId,
            branch: maintenance.branch || branch || '',
            location: maintenance.location || location || '',
            itemName: maintenance.itemName || itemName || '',
            frequency: maintenance.frequency || frequency || 'Weekly',
            inspectionTasks: maintenance.inspectionTasks || '',
            maintenanceSchedule: maintenance.maintenanceSchedule || {},
            inspectionTaskSchedules: maintenance.inspectionTaskSchedules || {}
          };
          currentFrequency = maintenance.frequency || frequency || 'Weekly';
          ...
          return; // Exit early since we loaded from API
        }
      }
    } catch (error) {
  ...
  // Fallback: Use URL parameters if provided (legacy support)
    ...
    if (calendarGridContainer) {
      calendarGridContainer.innerHTML = `
        <div class="empty-state">
          ...
          <a href="inspection.html" style="margin-top: 1rem; display: inline-block; color: #140958; text-decoration: underline;">← Back to List</a>
        </div>
      `;
    }
...
async function renderCalendarGrid() {
  ...
  itemsToDisplay.forEach((item, index) => {
    ...
    // Build link URL for inspection task (staff version uses inspectionasset.html)
    const taskLink = item.isTask && currentMaintenanceItem?._id 
      ? `inspectionasset.html?maintenanceId=${encodeURIComponent(currentMaintenanceItem._id)}&taskIndex=${index}&taskText=${encodeURIComponent(displayText)}`
      : '#';
